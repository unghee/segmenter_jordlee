...
GraphCut::GraphCut()
{
  initialized = false;
  processed = false;
  createAllRelations = true;     // create full connected - to avoid segmentation fault
}
...
bool GraphCut::init(surface::View *view)
{
  ...
  if(createAllRelations) {
    ...
    for(unsigned i=0; i<maxID; i++) {
      for(unsigned j=i+1; j<=maxID; j++) {
        ...
        for(int k=0; k<view->relations.size(); k++) {
...
/**
 * @brief Initialize graph with given relations.
 * @param rel Vector with all relations between nodes
 * @return Returns false, if initialization fails.
 */
bool GraphCut::init(unsigned nrNodes, std::vector<surface::Relation> &rel)
{
  printf("[GraphCut::init] Warning: Antiquated function!\n");
  if(rel.size() < 1)
    return false;
  ...
  if(createAllRelations) {
    ...
    for(unsigned i=0; i<relations.size(); i++) {
      if(relations[i].id_0 > maxID)
        maxID = relations[i].id_0;
      if(relations[i].id_1 > maxID)
        maxID = relations[i].id_1;
    }
    for(unsigned i=0; i<maxID; i++) {
      for(unsigned j=i+1; j<=maxID; j++) {
        bool foundRelation = false;
        for(int k=0; k<relations.size(); k++) {
          if((relations[k].id_0 == i && relations[k].id_1 == j) ||
             (relations[k].id_1 == i && relations[k].id_0 == j))
            foundRelation = true;
        }
        if(!foundRelation) {
          surface::Relation r;
 ...
 * @brief Graph Cutting
 */
void GraphCut::process(surface::View *view)
{
  if(!initialized) {
    printf("[GraphCut::process] Error: Graph is not initialized!\n");
    return;
...
void GraphCut::getResults(unsigned nr_models, 
                          std::vector< std::vector<unsigned> > &_clusters)
{
  if(!processed) {
    printf("[GraphCut::getResults] Error: Graph is not processed!\n");
    exit(-1);
  }
  ...
  unsigned cut_labels[nr_models];             // cut-ids for all models
  std::set<unsigned> graphCutLabels;          // all graph cut labels
...
void GraphCut::printResults()
{
  printf("[GraphCut::printResults]\n");
