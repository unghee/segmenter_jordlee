...
template<typename T>
void FileSystem::write_vector(const std::vector<T> &vec, FILE* pFile)
{
  unsigned s = vec.size();
...
void FileSystem::write_eigen_vector2d(
    const std::vector<Eigen::Vector2d, Eigen::aligned_allocator<Eigen::Vector2d> > &vec, FILE* pFile)
{
  unsigned s = vec.size();
  fwrite(&s, sizeof(unsigned), 1, pFile);
  for (unsigned i = 0; i < vec.size(); i++) {
...
void FileSystem::write_eigen_vector3d(
    const std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d> > &vec, FILE* pFile)
{
  unsigned s = vec.size();
  fwrite(&s, sizeof(unsigned), 1, pFile);
  for (unsigned i = 0; i < vec.size(); i++) {
...
void FileSystem::write_eigen_vector3f(
    const std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f> > &vec, FILE* pFile)
{
  unsigned s = vec.size();
  fwrite(&s, sizeof(unsigned), 1, pFile);
  for (unsigned i = 0; i < vec.size(); i++) {
...
void FileSystem::write_nurbs_surface(const ON_NurbsSurface &surf, FILE* pFile)
{
  ...
  int cv_capacity = surf.m_cv_count[0] * surf.m_cv_count[1] * surf.m_cv_stride[1];
  for (int i = 0; i < cv_capacity; i++) {
    fwrite(&surf.m_cv[i], sizeof(double), 1, pFile);
  }
}

void FileSystem::write_tgModel(const TomGine::tgModel &model, FILE* pFile)
{
...
void FileSystem::write_surface_model(const SurfaceModel &model, FILE *pFile)
{
  ...
  write_vector(model.contour, pFile);
  write_vector(model.error, pFile);
  write_vector(model.probs, pFile);
  write_vector(model.neighbors2D, pFile);
  write_vector(model.neighbors3D, pFile);
...
void FileSystem::write_view(const View &view, FILE *pFile)
{
...
template<typename T>
size_t FileSystem::read_vector(std::vector<T> &vec, FILE* pFile)
{
  size_t result(0);
  unsigned s;
...
size_t FileSystem::read_eigen_vector2d(std::vector<Eigen::Vector2d, Eigen::aligned_allocator<Eigen::Vector2d> > &vec,
    FILE* pFile)
{
  size_t result(0);
  unsigned s;
...
size_t FileSystem::read_eigen_vector3d(std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d> > &vec,
    FILE* pFile)
{
  size_t result(0);
  unsigned s;
...
size_t FileSystem::read_eigen_vector3f(std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f> > &vec,
    FILE* pFile)
{
  size_t result(0);
  unsigned s;
...
size_t FileSystem::read_nurbs_surface(ON_NurbsSurface &surf, FILE* pFile)
{
  size_t result(0);
...
size_t FileSystem::read_tgModel(TomGine::tgModel &model, FILE* pFile)
{
  size_t result(0);
...
size_t FileSystem::read_surface_model(SurfaceModel &model, FILE *pFile)
{
  size_t result(0);
  ...
  result += read_vector(model.contour, pFile);
  result += read_vector(model.error, pFile);
  result += read_vector(model.probs, pFile);
  result += read_vector(model.neighbors2D, pFile);
  result += read_vector(model.neighbors3D, pFile);
...
size_t FileSystem::read_view(View &view, FILE* pFile)
{
  size_t result(0);
...
bool FileSystem::Save(const std::vector<SurfaceModel> &models, const std::string &filename)
{
  FILE * pFile;
...
bool FileSystem::Load(std::vector<SurfaceModel> &models, const std::string &filename)
{
  FILE * pFile;
...
bool FileSystem::Save(const std::vector<SurfaceModel::Ptr> &models, const std::string &filename)
{
  FILE * pFile;
  pFile = fopen(filename.c_str(), "wb");
  if (pFile == NULL) {
...
bool FileSystem::Load(std::vector<SurfaceModel::Ptr> &models, const std::string &filename)
{
  FILE * pFile;
  pFile = fopen(filename.c_str(), "rb");
  if (pFile == NULL) {
...
bool FileSystem::Save(const View &view, const std::string &filename)
{
  FILE * pFile;
  pFile = fopen(filename.c_str(), "wb");
  if (pFile == NULL) {
    printf("[surface::FileSystem::Save(const View&, const std::string &)] Error: cannot open file for writing: '%s'\n",
        filename.c_str());
    return false;
  }
...
bool FileSystem::Load(View &view, const std::string &filename)
{
  FILE * pFile;
  pFile = fopen(filename.c_str(), "rb");
  if (pFile == NULL) {
...
// -------------------- Save File Sequence -------------------- //
SaveFileSequence::SaveFileSequence(Parameter p) : param(p)
{
  initialized = false;
...
void SaveFileSequence::InitFileSequence(std::string _filename, int _start, int _end)
{
...
void SaveFileSequence::SetCameraParameters(Eigen::Matrix3d &_intrinsic, 
...
void SaveFileSequence::SaveNextView(const std::vector<SurfaceModel> &surfaces)
{
  printf("[SaveFileSequence::SaveNext] Error: Not yet implemented. Do it NOW!\n");
}

void SaveFileSequence::SaveNextView(const std::vector<SurfaceModel::Ptr> &surfaces)
{
...
// -------------------- Load File Sequence -------------------- //
LoadFileSequence::LoadFileSequence(Parameter p) : param(p)
{}
...
void LoadFileSequence::InitFileSequence(std::string _filename, int _start, int _end)
{
  param.filename = _filename;
  param.start = _start;
  param.end = _end;
...
void LoadFileSequence::LoadNextView(View &view)
{
  if (current > param.end)
