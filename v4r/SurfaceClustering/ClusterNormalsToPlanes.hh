...
public:
  ...
  public:
    ...
    Parameter(float thrAngleNC=0.6, float _inlDist=0.02, int _minPoints=9, bool _adaptive=false, float _d_c = 0.0, 
              float _epsilon_c=0.58, float _epsilon_g=0.07, float _omega_c=-0.004, float _omega_g=0.015, float _ra_dist = 0.01) : 
...
private:
  ...
  boost::shared_ptr< std::vector<SurfaceModel::Ptr> > planes;
  ...
  cv::Mat_<unsigned> patches;      //< Patch indices(+1) on 2D image grid
...
public:
  ...
  /** Returns the estimated planes as surface models **/
  void getSurfaceModels(std::vector<SurfaceModel::Ptr> &_planes);
  
  /** Returs normals of the point cloud **/
  void getNormals(pcl::PointCloud<pcl::Normal>::Ptr &_normals) {_normals = normals;}
