...
{
  patches = cv::Mat_<unsigned>(cloud->height, cloud->width);
  for(unsigned i=0; i<cloud->height; i++)
    for(unsigned j=0; j<cloud->width; j++)
      patches(i,j) = 0;
...
{ 
  ...
  for(int su=0; su<(int)(*planes).size(); su++) {
    if((int)(*planes)[su]->indices.size() > 0) {
      if((int)(*planes)[su]->indices.size() < param.minPoints) {
        (*planes)[su]->type = -1;
...
{ 
  ...
  for(unsigned i=0; i<(*planes).size(); i++) {
    if(((*planes)[i]->indices.size() - reassign_idxs[i].size()) < param.minPoints)
      reassign_idxs[i] = (*planes)[i]->indices;
    else
                                            ...
                                            pcl::Normal &normal)
{
    ...
    for(unsigned i=0; i<n4ind.size(); i++) {
      ...
      if (v>=0 && u>=0 && v<height && u<width) {
        ...
        if (mask[idx]==0) {
          ...
          Eigen::Vector3f curPt = cloud.points[idx].getVector3fMap();
          if(param.adaptive) {
            if(curPt[2] <= param.d_c)
              newCosThrAngleNC = cos(param.epsilon_c);
            else
              newCosThrAngleNC = cos(param.epsilon_c + param.epsilon_g*(curPt[2]-param.d_c));
            newInlDist = param.omega_c + param.omega_g*curPt[2];
          }
  ...
  if( (int) pts.size() >= param.minPoints) {
    for(unsigned i=0; i<ptsSize; i++) {
      ...
      Eigen::Vector3f curPt = cloud.points[pts[i]].getVector3fMap();
        ...
        std::vector<SurfaceModel::Ptr> &planes)
{
  ...
  std::map<double, int> startPts;
  for(idx=0; idx<(unsigned) (width*height); idx++)
    startPts.insert(std::pair<double,int>(normals.points[idx].normal[4], idx));
  std::map<double, int>::iterator it = startPts.begin();
  
  // cluster points
  bool morePlanes = true;
  while(morePlanes) {
    morePlanes = false;
    for (it=startPts.begin() ; it!=startPts.end(); it++) {
      idx = (*it).second; 
      if (mask[idx]==0) {
  ...
  for (unsigned v=0; v<cloud.height; v++) {
    for (unsigned u=0; u<cloud.width; u++) {
      ...
      if (mask[idx] == 0) {
        ...
        if(isnan(cloud.points[idx].x))
          printf("[ClusterNormalsToPlanes::ClusterNormals] Error: Found NAN.\n");
...
{
  for(unsigned i=0; i<planes->size(); i++) {
    (*planes)[i]->normals.resize((*planes)[i]->indices.size());
    for(unsigned ni=0; ni<(*planes)[i]->indices.size(); ni++) {
      Eigen::Vector3f nor = normals->points[(*planes)[i]->indices[ni]].getNormalVector3fMap();
      (*planes)[i]->normals[ni][0] = nor[0];
      (*planes)[i]->normals[ni][1] = nor[1];
      (*planes)[i]->normals[ni][2] = nor[2];
    }
  }
}
 ...
 * Set input normals 
 */
void ClusterNormalsToPlanes::setInputNormals(const pcl::PointCloud<pcl::Normal>::Ptr &_normals)
{
  if (_normals->height<=1 || _normals->width<=1 || !_normals->isOrganized())
    throw std::runtime_error("[ClusterNormalsToPlanes::setInputNormals] Invalid normals (height must be > 1)");
...
void ClusterNormalsToPlanes::compute(const std::vector<int> &indices)
{
  ...
  if (cloud.get()==0 || normals.get()==0)
    throw std::runtime_error("[ClusterNormalsToPlanes::compute] Point cloud or normals not set!"); 

  planes.reset(new std::vector<SurfaceModel::Ptr>);

  ClusterNormals(*cloud, *normals, indices, *planes);
  if(pixel_check)
    PixelCheck();
 ...
 * getSurfaceModels
 */
void ClusterNormalsToPlanes::getSurfaceModels(std::vector<SurfaceModel::Ptr> &_planes)
{
  _planes = *planes;
}
