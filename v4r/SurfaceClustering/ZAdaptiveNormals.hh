...
#ifndef PCLA_ZADAPTIVE_NORMALS_HH
#define PCLA_ZADAPTIVE_NORMALS_HH
...
public:
  ...
  {
    public:
      double radius;            // inlier radius (restricts the kernel)
      int kernel;               // kernel radius [px] (unimportant, because of the support radius
      ...
      Parameter(double _radius=0.02, int _kernel=5, bool _adaptive=false, float _kappa=0.01, float _d = 0.0)
...
private:
  ...
  float ComputeNormal(pcl::PointCloud<pcl::PointXYZRGB> &cloud, std::vector<int> &indices, 
        Eigen::Vector4f &mean, Eigen::Matrix3f &cov, Eigen::Vector3f &eigen_values, 
        Eigen::Matrix3f &eigen_vectors, float &eigsum);
