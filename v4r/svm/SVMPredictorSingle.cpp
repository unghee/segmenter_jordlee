 ...
 */
SVMPredictorSingle::SVMPredictorSingle(const char* filename)
{
  max_nr_attr = 64;
...
bool SVMPredictorSingle::process(int type, std::vector<double> &vec, std::vector<double> &prob)
{
  ...
  for(unsigned idx = 0; idx < vec.size(); idx++) {
    node[idx].index = idx+1;                                  /// TODO Starts with 1?
...
/**
 * @brief Set scaling of relation vector.
 * @param _scale on/off
 * @param _param_filename Filename with scaling parameters
 */
void SVMPredictorSingle::setScaling(bool _scale, const char* _param_filename)
{
  scale = _scale;
  if(!scale) return;
  ...
  fp_restore = fopen(_param_filename, "r");
  if(fp_restore==NULL) {
    ...
    printf("[SVMPredictorSingle::setScaling] Opened file %s\n", _param_filename);
