...
protected:
  ...
  std::vector<float> m_lineWidth3D; // probability of 3D lines
  ...
  //  std::vector<TomGine::tgTextureModel> m_tex_models;
  std::vector<cv::Mat_<cv::Vec3b> > m_textures;
  std::vector<TomGine::tgTexture2D*> m_textures_gl;

  std::vector<TomGine::tgNurbsSurfacePatch> m_nurbsSurfaceData;
  std::vector<TomGine::tgNurbsSurface*> m_nurbsSurface;
...
public:
   ...
   *  @param pcl Point cloud as tgModel
   ...
   *  @return Unique id of the point cloud added. */
  int AddPointCloud(const std::vector<cv::Vec4f> &cloud);
  ...
  void SetModel(int id, const TomGine::tgTextureModel &model);
