                                ...
                                pcl::PointCloud<pcl::PointXYZRGB>::Ptr &cloud)
{
  ...
  cnt = 0;
  if(use_database_path)
    cnt += snprintf(depth_next + cnt, size - cnt, "%sdepth/%s", db_path.c_str(), current_pcd_file);
  else
    sprintf(depth_next, data_pcd_filename.c_str(), data_current);  

  // load color and depth image
  // printf("[KinectData::loadDepthImage] Load file: %s\n", color_next);
  printf("[KinectData::loadDepthImage] Load file: %s\n", depth_next);
...
void KinectData::setReadDataFromFile(std::string pcd_filename,
...
{
  if(!initialized) {
    printf("[KinectData::loadImageData] Error: Loading not initailised. Abort\n");
    return;
  }
  ...
  if(!read_live) {
    ...
    if(use_database_path) {
      ...
      printf("[KinectData::loadImageData] Load file: %s\n", pcd_next);
    }
  ...
  else
  {
    ...
    pclA::ConvertCvMat2PCLCloud(mat_cloud, cloud);
    c = cloud;
  }
}
