...
double getEdgeLengthSqd(Edge* edge){
...
// remove too long boundary edges
void Triangulation::removeBoundaryTriangles(double min_edge_length){

  list<Edge*>::iterator it;
  unsigned n_long_edges(1);
  min_edge_length = min_edge_length * min_edge_length;

  while (n_long_edges > 0) {

    n_long_edges = 0;
    list<Edge*>* bnd_edges = this->getBoundaryEdges();

    for (it = bnd_edges->begin(); it != bnd_edges->end(); it++) {

      Edge* e0 = (*it);
      Edge* e2 = e0->getNextEdgeInFace()->getNextEdgeInFace();

      // check if edge is too long
      if (getEdgeLengthSqd(e0) > min_edge_length) {

//        if (getEdgeLengthSqd(e0) > getEdgeLengthSqd(e1) &&
//            getEdgeLengthSqd(e0) > getEdgeLengthSqd(e2)) {

        // get dart with node opposite to the boundary edge
        Dart d0 = Dart(e2);
        Dart d = d0;

        if( ttl::isBoundaryEdge(d) )
          continue;

        bool removeable = true;
        d.alpha2();
        d.alpha1();
        while(removeable && d != d0){
          if( ttl::isBoundaryEdge(d) ){
            removeable = false;
          }else{
            d.alpha2();
...
//--------------------------------------------------------------------------------------------------
list<Edge*>* Triangulation::getBoundaryEdges() const {
...
void Triangulation::swapEdge(Edge& diagonal) {
  ...
  Handle<Node> nR = eR_2->getSourceNode();
  Handle<Node> nL = eL_2->getSourceNode();
